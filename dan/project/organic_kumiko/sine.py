...
def position_fn(index, offset=Vec3()):
    return Vec3(index*20, math.sin(index*.7) * 30, 0) + offset
...
def make_poly(verts):
    ...
    pco.AddPath( pyclipper.scale_to_clipper( [[v.x, v.y] for v in verts] ), pyclipper.JT_SQUARE, pyclipper.ET_CLOSEDLINE )

    p2 = pyclipper.scale_from_clipper( pco.Execute( pyclipper.scale_to_clipper( 1 ) ) )
    part = linear_extrude( height=10 ) (
        polygon( points=p2[0] )
    ) - down( 1 ) ( 
...
lines = [
    ...
    [position_fn(i//2, offset=Vec3(0, 20, 0) if i % 2 else Vec3()) for i in range(40)],
]
