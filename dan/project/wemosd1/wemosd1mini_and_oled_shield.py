...
class ScrewHoleBlock(Part):
    block = CubePart(6, 6, 5).cut(
        Rod(r=1.5, h=5+.1).translate(2.3+1, 2.3+1, 0)
    )

class ButtonHole(Part):
    block = Rod(r=3, h=wall_thickness+1, segments=24)
    inner = Rod(r=4, h=wall_thickness, segments=24).translate(0, 0, .5 + wall_thickness*.5)
...
class Box(Part):
    box = Box(
        ...
        CubePart(15, 5, 10).translate(wall_thickness+box_inner_size.x*.5-7.5, -1, wall_thickness+3)
    ).cut(
        CubePart(box_inner_size.x + 2, box_inner_size.y + 2, 3).translate(wall_thickness-1, wall_thickness-1, box_inner_size.z+wall_thickness-2)
    )

    box.cut(
        ButtonHole().rotate(v=[0, 1, 0], a=90).translate(-.5, wall_thickness+4, wall_thickness+17)
    )

    box.cut(
        ButtonHole().rotate(v=[0, 1, 0], a=-90).translate(box_inner_size.x+wall_thickness*2, wall_thickness+4, wall_thickness+17)
    )
