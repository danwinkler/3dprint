...
def case_hole():
    ...
    return (translate( [-(hole_offset_in_inches), -(hole_offset_in_inches), 0]) (
        cube( [hole_offset_in_inches*2, hole_offset_in_inches, height]),
        cube( [hole_offset_in_inches, hole_offset_in_inches*2, height]),
    ) + cylinder( r=hole_offset_in_inches, h=height, segments=24 )) + hole() (
        down( round_over+.1) (
            cylinder(r=hole_size_in_inches*.5, h=height+.3, segments=24),
        )
    )
...
def case():
    outer_case = minkowski()(
        ...
        cube([width_in_inches+round_over*2, length_in_inches+round_over*2+.2, round_over*2])
    )
    ...
    screen_hole = translate([round_over+width_in_inches-1-.2, round_over+length_in_inches*.5 - .5, -.1]) (
        hole() ( cube([1, 1, 1]) )
    )

    temp_slot = translate([(round_over*2+width_in_inches) - (round_over+width_in_inches*.5 - .9), round_over+length_in_inches+round_over, top_height - (1.0/8.0)]) (
        hole() (
            translate([-.60 - 1.02, 0, 0]) (
