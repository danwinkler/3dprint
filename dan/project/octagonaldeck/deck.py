...
first_support_height = deck_height - first_level_support_size - second_level_support_size - inches(2)
second_support_height = first_support_height + first_level_support_size

radius = side * math.sqrt( 4 + 2 * math.sqrt(2) ) / 2
medium_diagonal = math.cos( math.pi*2/8 * .5 ) * radius * 2

useable_radius = radius - fire_pit_radius - inches( 5 )
num_rings = int(useable_radius/inches(1/(math.cos(math.pi*.125)/6.1)))
...
if show_fire_pit:
    parts.append(
        rotate( rot_offset*delta_a ) (
            color( [.5,.5,.6] ) (
                cylinder( r=fire_pit_radius, h=fire_pit_height_above_deck+deck_height, segments=(8 if fire_pit_is_octagon else 20) ) -
...
# Concrete supports
for a in xrange(8):
    parts.append( rotate( a=(a+rot_offset)*delta_a ) (
        right( radius ) (
...
if inside_supports:
    for x, y in [[1,1],[-1,1],[1,-1],[-1,-1]]:
        parts.append( translate( [side/2 * x, side/2 * y, 0] ) (
            color( [1,0,0] ) ( cylinder( r=support_rad, h=support_height, segments=8 ) ),
            translate( [-inches(2), -inches(2), support_height] ) ( cube( [inches(4), inches(4), first_support_height + first_level_support_size - support_height] ) )
        ))
...
for a in xrange(8):
    ...
    parts.append(translate( [x,y,0] ) (
        translate( [-inches(2),-inches(2),support_height] ) (
            cube( [inches(4), inches(4), deck_height + railing_height - support_height] )
        )
    ))
...
if show_level_1:
    ...
    parts.append(
        translate( [-side/2 - inches(4), -medium_diagonal/2 - inches( 2 ), first_support_height] ) (
            cube( [inches(2),long_board_length, first_level_support_size] )
        )
    )
    parts.append(
        translate( [side/2 + inches(2), -medium_diagonal/2 - inches( 2 ), first_support_height] ) (
            cube( [inches(2), long_board_length, first_level_support_size] )
        )
    )
    ...
    # Short
    parts.append(
        translate( [-medium_diagonal/2 - inches( 4 ), -side/2 - inches(2), first_support_height] ) (
            cube( [inches(2), short_board_length, first_level_support_size] )
        )
    )
    parts.append(
        translate( [medium_diagonal/2 + inches( 2 ), -side/2 - inches(2), first_support_height] ) (
            cube( [inches(2), short_board_length, first_level_support_size] )
        )
    )
    ...
    # Outer Boards
    x_axis.append(
        translate( [-medium_diagonal/2 - inches( 4 ), -side/2 -inches(4),first_support_height] ) (
    ...
    x_axis.append(
        translate( [medium_diagonal/2 - outer_board_length + inches(4), -side/2 -inches(4),first_support_height] ) (
            cube( [outer_board_length,inches(2),first_level_support_size] )
        )
    )

    x_axis.append(
        translate( [medium_diagonal/2 - outer_board_length + inches(4), side/2+inches(2),first_support_height] ) (
            cube( [outer_board_length,inches(2),first_level_support_size] )
        )
    )
    ...
    # End Boards
    x_axis.append(
        translate( [-side/2 - inches( 4 ), -medium_diagonal/2 - inches(4), first_support_height] ) (
    ...
    for i in xrange(1, x_num_boards):
        #Left
        x_axis.append(
            translate( [-medium_diagonal/2 - inches( 2 ), -side/2 -inches(3) + i*x_sep,first_support_height] ) (
                cube( [outer_board_length-inches(2),inches(2),first_level_support_size] )
            )
        )
        #Right
        x_axis.append(
            translate( [medium_diagonal/2 - outer_board_length + inches( 4 ), -side/2 -inches(3) + i*x_sep,first_support_height] ) (
                cube( [outer_board_length-inches(2),inches(2),first_level_support_size] )
            )
        )
    ...
    for i in xrange( 1, y_num_boards ):
        #Bottom
        x_axis.append(
            translate( [-side/2 - inches( 2 ), -medium_diagonal/2 - inches(4) + i*y_sep, first_support_height] ) (
                cube( [end_board_length-inches(4),inches(2),first_level_support_size] )
            )
        )
        #Top
        x_axis.append(
            translate( [-side/2 - inches( 2 ), medium_diagonal/2 + inches(2) - i*y_sep, first_support_height] ) (
                cube( [end_board_length-inches(4),inches(2),first_level_support_size] )
            )
        )
...
if show_level_2:
    def cross_piece( d ):
        bsx = d
        bsy = 0
        bex = math.cos( math.pi*.125 ) * bsx
        bey = math.sin( math.pi*.125 ) * bsx
    ...
    for a in xrange(8):
        ...
        # To face
        parts.append( rotate( a=(a)*delta_a ) (
            translate( [fire_pit_radius + inches(2),0,second_support_height] ) (
                color( [1,1,0] ) (
                    cube( [medium_diagonal/2 - fire_pit_radius + inches( 2 ), inches(2), second_level_support_size] )
                )
            )
        ))
    ...
    for a in xrange(16):
        ...
        parts.append( rotate( a=(a)*delta_a*.5 + delta_a*.25 ) (
            translate( [medium_diagonal/2,-o_board_length/2 + inches(2),second_support_height] ) (
                color( [1,.5,0] ) (
                    cube( [inches(2), o_board_length-inches(2), second_level_support_size] )
                )
            )
        ))
        ...
        #Small Fillers
        parts.append( rotate( a=(a)*delta_a*.5 + delta_a*.25 ) (
            translate( [fire_pit_radius + inches(2),-board_length/2 + inches(2),second_support_height] ) (
                color( [1,.5,0] ) (
                    cube( [inches(2), board_length-inches(2), second_level_support_size] )
                )
            )
        ))
...
if show_level_3:
    for r in xrange(num_rings):
        bsx = 0
        bsy = fire_pit_radius+inches(2) + useable_radius - r*inches(1/(math.cos(math.pi*.125)/6.1))
        bex = math.cos( math.pi*.25 ) * bsy
        bey = math.sin( math.pi*.25 ) * bsy
        ...
        for a in xrange(8):
            ...
            parts.append(
                rotate( a=(a+rot_offset)*delta_a ) (
                    difference() (
                        translate( [fire_pit_radius + inches(2) + useable_radius - r*inches(1/(math.cos(math.pi*.125)/6.1)) - inches(6),0,deck_height-inches(2)] ) (
                            rotate( a=22.5 ) (
                                color( [.7,.4,.2] ) (
                                    forward( -inches(2) ) (
                                        cube( [inches(6), board_length+inches(2), inches(2)] )
                                    )
                                )
                            )
                        ),
                        union() (
                            back( inches(4.1) ) ( cube( [100, inches(5), 100] ) )
                        )
                    )
                ) - rotate( a=((a+1)+rot_offset)*delta_a ) (
                    forward( inches(.4) ) ( cube( [10, inches(5+1), 6] ) )
                )
            )
