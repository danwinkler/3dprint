...
front_wall = up( wall_thickness ) (
    cube( [wall_thickness, outer_y, length])
)
...
floor = up( wall_thickness ) (
    cube( [outer_x+side_length, wall_thickness, length] )
)
...
holes = hole() (
    translate( [0, wall_thickness+inner_x*.5, wall_thickness+length*.5] ) (
        rotate( v=[0, 1, 0], a=-90 ) (
            screwhole( "#8 Wood", wall_thickness+1 )
        )
    ),
    translate( [outer_x*.5, 0, outer_y*.5-5] ) (
        rotate( v=[1, 0, 0], a=90 ) (
            screwhole( "#8 Wood", wall_thickness+1 )
        )
    )
