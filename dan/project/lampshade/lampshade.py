...
def r( a, z ):
	...
	ret += 30-(1.0/((z*.3)+1))*30 #The little curve next to the hole
	ret += (1.0/((v_sections-z)+2))*30 #curve at the base
	if z >= 1:
		ret += (math.sin( a*12 ) * 1.5)
		ret += (math.cos( z*.25*v_scale ) * 1.5)
	return ret
...
for z in range( v_sections ):
	...
	for i in range( h_sections ): 
		...
		point = [math.sin( angle ) * r( angle, z ), math.cos( angle ) * r( angle, z ), z*v_scale]
		if z == 0:
			point[2] = v_scale
		points.append( point )
	rings.append( points )
		...
		if debug:
			...
			parts.append(
				hull() (
					...
					translate( rings[z+1][(i+1)%h_sections] ) ( sphere( wall_thickness ) )
				)
			)
