...
parts = [
    ...
    up( height ) (
        ...
        cube( [wall_thickness*2+wood_thickness, wall_thickness+length, wall_thickness] )
        ),
        translate( [wall_thickness+wood_thickness, 0, 0] ) (
            cube( [wall_thickness+length, wall_thickness*2+wood_thickness, wall_thickness] )
        )
    ),
    #floor
    translate( [bod_size, bod_size, 0] ) (
        cube( [length, length, wall_thickness] )
    ),
    #screwholes
    translate( [bod_size+length*.5, 0, height*.5] ) (
        rotate( a=90, v=[1, 0, 0] ) (
            screwhole( "#8 Wood", bod_size+1 )
        )
    ),
    translate( [0, bod_size+length*.5, height*.5] ) (
        rotate( a=-90, v=[0, 1, 0] ) (
            screwhole( "#8 Wood", bod_size+1 )
        )
    ),
    translate( [bod_size+length*.5, bod_size+length*.5, wall_thickness+5] ) (
