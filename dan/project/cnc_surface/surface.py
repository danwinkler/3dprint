...
def blob( x, y ):
    ...
    xs = ((w2 - abs(x-w2)) / w2)
    ys = ((h2 - abs(y-h2)) / h2)
    return 50.0 / (1+10*math.e**-(.1*(xs*ys*50)))
...
def sin( x, y ):
    ...
    xs = ((w2 - abs(x-w2)) / w2)
    ys = ((h2 - abs(y-h2)) / h2)
    return 50 * math.sin( xs * math.pi*.5 ) * math.sin( ys * math.pi*.5 )
...
for x in range(x_sections):
    ...
    for cut_y in range(y_sections):
        ...
        p.append( [y_mid - material_thickness*.5, 0] )
        p.append( [y_mid - material_thickness*.5, cut_depth] )
        p.append( [y_mid + material_thickness*.5, cut_depth] )
        p.append( [y_mid + material_thickness*.5, 0] )
...
for y in range(y_sections):
    ...
    for i in range( x_sections ):
        ...
        x1 = x + material_thickness*.5
        x2 = x - material_thickness*.5
