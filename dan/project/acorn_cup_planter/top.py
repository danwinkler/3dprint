...
profile_shapes = [
    ...
    translate([cup_radius + t2, rim_height])(
        sphere(r=t2, segments=16)
    ),
    # Top outer edge of rim
    translate([cup_radius + t2, t2])(
        sphere(r=t2, segments=16)
    ),
    # Inner edge of rim
    translate([cup_radius - cup_thickness - t2, t2])(
        sphere(r=t2, segments=16)
    ),
    # Connection to center acorn holder
    translate([acorn_holder_size/2 + t2, bowl_depth + 2])(
        sphere(r=t2, segments=16)
    ),
]
...
holder = up(bowl_depth)(
    (sphere(r=acorn_holder_size/2 + thickness, segments=segments) + hole()(sphere(r=acorn_holder_size/2, segments=segments))) - down(acorn_holder_size * .8)(sphere(r=acorn_holder_size, segments=segments))
)
...
holes = [
    rotate(a=360 / num_holes * i, v=[0, 0, 1]) (
        translate([acorn_holder_size * .6, 0, 0]) (
            hull() (
